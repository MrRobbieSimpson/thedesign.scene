...
async function FooterJoinCtaSlot() {
  const subscribedToDigest = await isCurrentUserOnDigest();
  return <FooterJoinCta subscribedToDigest={subscribedToDigest} />;
}
    ...
    <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
      <div className="space-y-2">
        ...
        <p className="max-w-sm text-sm leading-relaxed text-muted-foreground">
          A considered selection of writing, visuals, and events — quality over
          quantity.
        </p>
        {designers > 30 ? (
  ...
  return (
    <footer className="mt-auto border-t border-border/60">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-5 py-12 sm:px-6">
        <Suspense fallback={<FooterJoinCta />}>
          <FooterJoinCtaSlot />
        </Suspense>
